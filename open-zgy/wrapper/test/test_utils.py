...
def SDTestData(name):
    ...
    """
    project = os.getenv("OPENZGY_SDTESTDATA", "sd://opendes/slb-testdata")
...
def SDTestSink(name):
    ...
    """
    project = os.getenv("OPENZGY_SDTESTSINK", "sd://opendes/slb-testsink/a")
    if project and project[-1] == "/": project = project[:-1]
    return project + "/" + name if name else project
...
def HasSeismicStore():
    ...
    TODO-Test: structure the tests better, to make it less likely to need this.
    """
...
class TempFileAutoDelete:
    ...
    def remove(name, iocontext = None, raise_on_error = True, silent = False):
        ...
        if not iocontext:
            credentials = ("", "", "")
        elif hasattr(iocontext,"sdtoken"):
        ...
        if name:
            try:
                if name and name[:5] == "sd://":
                    utils = api.ZgyUtils("sd://", SDCredentials())
                    utils.delete(name, missing_ok=False)
                else:
                    os.remove(name)
    ...
    def _randomname():
        try:
            with open("/dev/urandom", "rb") as urandom:
                n = np.frombuffer(urandom.read(4), dtype=np.uint32)[0]
...
class ProgressWithDots:
    """
    It is fairly pointless to wrap the C++ version if the progress reporter
    so I will just re-implement it, copy/paste from OpenZGY/Python.
    """
