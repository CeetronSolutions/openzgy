...
Naming conventions used in this file for parameters:
    ...
    bricksize: (i,j,k) -- Hos data is stored. Usually (64, 64, 64).
...
import numpy as np
from openzgycpp import SampleDataType

def _map_SampleDataTypeToNumpyType(dt):
    return {SampleDataType.int8:     np.int8,
            SampleDataType.int16:   np.int16,
            SampleDataType.float32: np.float32}[dt]
...
def myreader(r, pos, data, lod):
    if data.flags.c_contiguous:
        return r.read(pos, data, lod=lod)
    else:
        tmp = np.copy(data)
        rc = r.read(pos, tmp, lod=lod)
        data[:,:,:] = tmp
        return rc
...
def readall(r, *, blocksize = None, chunksize=None, maxbytes=128*1024*1024, dtype=None, cropsize = None, cropoffset = None, lod = 0, sizeonly = False, progress=None):
    ...
    if sizeonly:
        ...
        readfn = lambda pos, data: myreader(r, pos, data, lod)
    else:
    ...
    dtype = dtype or { SampleDataType.int8:    np.int8,
                       SampleDataType.int16:   np.int16,
                       SampleDataType.float32: np.float32 }[dt]
